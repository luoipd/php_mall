...
class StatisticalService
{
    ...
    public static function GoodsHotSaleSevenTodayTotal($params = [])
    {
        ...
        $data = Db::name('Order')->alias('o')->join(['__ORDER_DETAIL__'=>'od'], 'o.id=od.order_id')->where($where)->field('od.title AS name,sum(od.buy_number) AS value')->order('value desc')->limit(10)->group('od.title')->select();
        if(!empty($data))
        {
            foreach($data as &$v)
            {
